...
void bei(List *l, char *entrada, int i, int j)//i=[//j=]
{
    ...
    for (k = i+1; k < j; k++)
    {
       if(entrada[k]!=' ' && flag==0){
            insert(l, entrada[k]);
            entrada[k]=' '; entrada[i]=' '; entrada[j]=' ';
            }
    }

}

void beiju(List *l, char *entrada, int i,int tam)
{ // 5

    int j = 0, flag = 1;

    for (j = i + 1; j < tam; j++)//encontrando o mais perto
    {
        if (entrada[j] == ']')
        {
            bei(l, entrada, i, j);
...
List *beijuText(List *l, char *entrada)
{
    ...
    int i = 0, flag = -1; int tam = strlen(entrada);

    while (i < tam)
    {
        if (entrada[i] == '[')
        {                       
            if(entrada[i+1]==']') {entrada[i]=' '; entrada[i+1]=' ';}
            else beiju(l, entrada, i,tam); // 5
        }
        i++;
    }
